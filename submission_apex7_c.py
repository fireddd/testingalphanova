"""Competition 5 submission: APEX-7 — APEX-P plus a corrected inner beta projection.

Forecast machinery (unchanged from APEX6 / APEX-P):

  0.60  Apex4 core   (5 warm-started legs + inner betaneut + 0.25 reversal-neut)
  0.20  GBM leg      (4-model recency-weighted LightGBM ensemble)
  0.20  TF leg       (per-ticker autocorrelation heterogeneity)
then one rolling-beta projection of the COMBINED signal, then the APEX-P
risk-model position map, then row-demean.

INHERITED FROM APEX-P — the risk-model position mapping.  The final forecast
vector is mapped to positions through an inverse covariance rather than being
used directly (Kakushadze eq. 59: the unconstrained Sharpe-maximising holdings
are w = C^-1 f).  C is a causal rolling 250-bar covariance of Feature.1 across
the 20 names with Ledoit-Wolf shrinkage toward a scaled identity (the LW
intensity is computed analytically from the data — no fitted hyper-parameter).
The mapped vector is rescaled to the row L2 norm of the input (purely
directional — no implicit per-bar vol timing) and blended 50/50 with the
untransformed forecast.  Constants: LAM = 0.50, COV_WIN = 250, COV_MINP = 60.

NEW IN APEX-7 (the only difference vs submission_apexP.py — 4 lines changed,
1 line added, inside _Apex4.predict):  the INNER beta neutralisation projects
onto the ROW-DEMEANED beta column Bd = B - rowmean(B) instead of the raw B.

  Why this is a bug fix and not a tuning knob.  B_j = cov(f1_j, mkt)/var(mkt)
  with mkt = mean_j f1_j, so sum_j B_j = J exactly: rowmean(B) is identically
  1.0 (measured 1.0000000, std 3.8e-07 over all 61,243 rows).  Write
  B = 1 + Bd.  The forecast `out` is exactly row-demeaned at that point, so
  <out,B> = <out,Bd>; but the denominator keeps the constant part,
  <B,B> = J + <Bd,Bd> = 20 + <Bd,Bd>.  The removed vector is therefore
  (<out,Bd>/(20 + <Bd,Bd>)) * B, whose constant component is then annihilated
  by the mandatory row-demean.  Net effect: the code removed only the fraction
  s = <Bd,Bd>/(20 + <Bd,Bd>) of the beta exposure it was written to remove —
  median 4.2%, mean 6.8%.  Projecting on Bd makes the operation an actual
  orthogonal projection.  ZERO new constants; nothing is fitted or selected.

  Applied at the INNER site only.  self.last_B stays the RAW B and the combined
  beta neutralisation in Apex7Predictor.predict is deliberately left on raw B:
  applying the same correction at the combined site measures +0.0000 on h2 and
  fails the required bootstrap (p_h2 = 0.53 vs 0.92), because after a correct
  inner projection the combined projection over-neutralises an exposure the
  GBM/TF legs only partly reintroduce.

Local evidence (77-period walk-forward, prediction-level replay; two
independent rebuilds; fidelity gate vs cached apex6 PnL max|delta| 3.5e-09):

              full    sel75      h1      h2   rec20   hold(sealed)
  apex6     0.1115   0.1121  0.1454  0.0683  0.0852  0.0942
  apexP     0.1158   0.1163  0.1484  0.0745  0.0901  0.1002
  APEX7     0.1191   0.1195  0.1521  0.0768  0.0949  0.1071

Window-restricted period bootstrap, 2000 resamples, pools resampled separately:
  vs apex6:  h2 +0.0085 p=1.000    rec20 +0.0097 p=0.996
  vs apexP:  h2 +0.0023 p=0.92     rec20 +0.0048 p=0.99   (h1 is the WEAKEST
  pool, p=0.90 — this is not an h1 artefact.)  Leave-one-period-out jackknife:
  0/37 negative on h2, 0/20 negative on rec20.  The gain is a broad plateau —
  every kappa in [0.4, 1.0] of the partial-to-full projection clears both bars;
  kappa = 1 (a true projection) is the zero-parameter endpoint.

Harness (runner.py --full --gauge-fix): Sharpe 0.1191, IC 0.0391,
concentration 0.0519, city novelty 36.8 deg (nearest nonius_0268).
pnl-corr vs apex6 0.916, apexP 0.944, sparse 0.341, unique 0.253, ticket1 0.354.

Honest caveats: the point gain over apexP (+0.0032 sel75) is ~0.35x the
selection floor a single free parameter would buy; the h2 p-value sits near
0.90 after correcting for the 3-way (inner / combined / both) site choice; and
IC falls monotonically (0.0446 -> 0.0414 -> 0.0391) while Sharpe rises, so if
the platform scores something closer to IC than to Sharpe this is a regression.
"""
...
class Apex7PredictorC(Predictor):
