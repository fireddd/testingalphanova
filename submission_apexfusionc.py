"""Competition 5 submission: "apexfusion" — THREE legs, equal risk, no fitted weights.

    sig = L_t2 / SCALE_T2 + L_resid / SCALE_RESID + L_apex6 / SCALE_APEX6,
    then per-row cross-sectional de-mean.

This is submission_t2resid.py (the LIVE-lineage two-leg build) with the apex6
flagship added as a third INDEPENDENT family, at the same frozen equal-risk
weighting scheme.  Nothing here is fitted, tuned, or argmaxed.
  ...
  L_t2    = ticket2's full signal (price-space VAR(1) lead-lag graph + rank-space
            ...
            nested helper `_Ticket2`.  The `_legA` body is the vectorised form
            that was verified BIT-IDENTICAL (max|delta| = 0.0, exact equality)
            to the original row-at-a-time recursion; it is a pure speed rewrite.
  L_resid = -csrank( per-row cross-sectional OLS residual of Feature.1 on
            [1, F2..F6] ).  Row-local, stateless, causal.
  L_apex6 = submission_apex6.Apex6Predictor's signal (0.60 apex4 core +
            0.20 LightGBM ensemble + 0.20 per-ticker autocorrelation leg, with a
            combined rolling-beta neutralisation), inlined as the non-inheriting
            nested helper `_Apex6`.  Copied line-for-line from the shipped,
            platform-ACCEPTED apex6 (platform 0.0477) so that its measured
            transfer ratio (0.428) remains a valid anchor for this blend, with
            EXACTLY ONE DELIBERATE DEVIATION:

              apex6   :  pm = pm / (pm.std() + 1e-12)
              here    :  pm = pm / (pm.std(axis=1, keepdims=True) + 1e-12)

            The un-axised form is a std over the WHOLE prediction block, i.e. a
            look-ahead statistic.  Because the four GBM models are each rescaled
            by a different whole-block constant before summing, it perturbs the
            cross-sectional RANKING of earlier rows, not merely a global scale:
            an element-wise causality probe showed 3.7%-29.7% of pre-cut rows
            reordered by future data (0/6 probe cells exact-zero).  With axis=1
            the probe is 6/6 exact 0.0 on every leg.  The change is free and
            score-neutral-to-positive: harness 0.130597 vs 0.130469, and equal
            or better on all six windows including the sealed holdout.
            `submission_apex6.py` itself is NOT modified, so the 0.0477 anchor
            stands.  NOTE: this one line means the apex6 sleeve here is not
            bit-identical to the shipped apex6 (PnL corr 0.99981).

SCALES are FROZEN CLASS CONSTANTS (per-leg sel75 PnL volatility, ddof=0,
measured once offline from the cached per-leg PnL series), NOT re-measured in
train(); train() prepares warm-up state only and measures nothing over the
scored block.  NO fitted blend weight, NO argmax, NO tuning.

CAUSALITY: train() reads training rows only.  L_resid is a per-row
cross-sectional OLS residual (row-local); L_t2 recurses causally forward; the
apex6 sleeve uses only expanding/rolling causal statistics warm-started from
training-row sufficient statistics.  csrank and de-mean are per-timestamp
cross-sectional operations.  No backward fill, no negative shift, no whole-block
statistic feeding a prior row.

LEGALITY: exactly one Predictor subclass at module level; every helper is a
nested, non-inheriting class (the established pattern, cf. submission_apex8).
"""
...
class ApexFusionPredictor(Predictor):
    """ticket2 + cross-sectional resid + apex6, frozen equal-risk blend."""
    ...
    SCALE_APEX6 = 0.024651576010626338  # frozen-scale: apex6 full-signal PnL vol

    # ====================================================================
    #  ticket2 sleeve — verbatim from submission_t2resid_fast (whose _legA is
    #  a verified bit-identical vectorisation of submission_ticket2's loop),
    ...
    # ====================================================================
    #  apex6 sleeve — line-for-line copy of submission_apex6.Apex6Predictor
    #  (re-indented only), stripped of the `Predictor` base so that this
    #  module still exposes exactly ONE Predictor subclass.  ONE deliberate
    #  deviation: `_GBM.predict` uses `pm.std(axis=1, keepdims=True)` where
    #  apex6 has the un-axised `pm.std()`.  That whole-block std is a genuine
    #  look-ahead (it reordered 3.7-29.7% of pre-cut rows in an element-wise
    #  causality probe); axis=1 makes it row-wise and the probe goes 6/6 exact
    #  zero.  See the module docstring.  Do NOT revert it.
    # ====================================================================
    class _Apex6:
        """0.60*apex4 + 0.20*gbm + 0.20*tf, fixed scales, combined beta-neut."""
    ...
    def __init__(self):
        ...
        self._a6 = self._Apex6()

    def train(self, features: pd.DataFrame, target: pd.DataFrame) -> None:
        # WARM-UP STATE ONLY (no scale measurement, no blend fitting):
        #   * ticket2 sleeve stores leg A's bufA_ and leg B's WB_.
        #   * apex6 sleeve stores its apex4 prior sufficient statistics + the
        ...
        self._a6.train(features, target)
        tk = self._tickers(features)
        self.tk_ = tk
        self.j_ = len(tk)
        self.trained = True
    ...
    def predict(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        L_t2 = pt2[tk].to_numpy(dtype=np.float64)   # identical ticker order
        # L_apex6 (apex6 full signal)
        pa6 = self._a6.predict(features)
