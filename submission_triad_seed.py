...
class TriadSeedPredictor(Predictor):
    ...
    class _MLLeg:
        ...
        class _GBM:
            ...
            def train(self, features: pd.DataFrame, target: pd.DataFrame) -> None:
                ...
                T = X3.shape[0]
                self.models = []
                for _mi, cfg in enumerate(self.CONFIGS):
                    t0 = max(0, T - cfg["cap"])
                    Xs = X3[t0:]
                    ynext = f1rank[t0 + 1:]
                    ...
                    _pr = dict(self.LGB_PARAMS)
                    _pr["seed"] = 7 + 4 * _mi   # pre-declared: decorrelate sampling noise
                    mdl = lgb.train(_pr, ds, num_boost_round=self.N_ROUNDS)
