"""Competition 5 — basic gradient-boosted cross-sectional ranker (XGBoost).
...
class GbmXgbPredictor(Predictor):
    """One XGBoost model on rank features -> csrank -> row de-mean."""
    ...
    def train(self, features: pd.DataFrame, target: pd.DataFrame) -> None:
        ...
        self.model = xgb.XGBRegressor(
            n_estimators=self.ROUNDS, max_depth=4, learning_rate=self.LR,
            subsample=0.8, colsample_bytree=0.8, reg_lambda=5.0,
            min_child_weight=50, random_state=self.SEED, n_jobs=2,
