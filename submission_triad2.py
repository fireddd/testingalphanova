"""Competition 5 submission — TWO independent legs, equal risk, no fitted weights.

    sig = L_graph / SCALE_GRAPH + L_ml / SCALE_ML,
    then per-row cross-sectional de-mean.

This is `submission_triad.py` with the cross-sectional-residual leg DELETED and
NOTHING else changed: the two surviving legs are byte-identical to the triad's,
and their two frozen equal-risk scales are the SAME constants (each leg is still
divided by its own first-75-period realised PnL volatility, so no scale needed
re-measuring — no constant in this file is new).

WHY THESE TWO LEGS.  They are the only two components of the triad whose PLATFORM
transfer is DIRECTLY MEASURED on the exact object that appears as the leg:

    L_graph  is bit-identical to submission_ticket2.py   platform 0.0710 / local
             0.0922  ->  transfer 0.7701   (the highest ever measured here)
    L_ml     is bit-identical to submission_apex6.py     platform 0.0477 / local
             0.1115  ->  transfer 0.4278

The deleted residual leg has never been submitted; its transfer is only inferable
(0.5078, pinned by t2resid's 0.0731/0.1123 given the graph leg's 0.7701).  Removing
it therefore both raises the share of the best-transferring leg and removes the one
component of the projection that rests on an unmeasured quantity.
...
SCALE_GRAPH is verified equal to submission_ticket2's own sel75 PnL volatility to
all 17 significant digits, so "equal risk" here is the same normalisation the
triad used, unchanged by the leg removal.

CAUSALITY.  train() reads training rows only.  L_graph recurses causally forward
one row at a time from a training-row warm-up buffer.  L_ml uses only expanding /
rolling statistics warm-started from training-row sufficient statistics; every
normalisation inside it is row-wise, so no statistic computed over the scored
block can feed an earlier row.  csrank and de-mean are per-timestamp
cross-sectional operations.  No backward fill, no negative shift, no whole-block
statistic.  An element-wise causality probe (perturb rows >= t, check rows < t)
returns exactly 0.0 on every leg and on the fused output.
...
HARDCODED NUMERIC LITERALS (all labelled):
    frozen-scale : SCALE_GRAPH, SCALE_ML (per-leg PnL vols, unchanged from
                   submission_triad.py).
...
class Triad2Predictor(Predictor):
    """Lead-lag graph + learned leg, frozen equal risk, no fitted weight."""

    # ---- FROZEN equal-risk scales (per-leg sel75 PnL vol, ddof=0) ----------
    # sel75 = concatenated validation rows of periods 0..74 = rows [0:59643].
    # Both values are copied unchanged from submission_triad.py; the legs they
    # normalise are unchanged, so neither needed re-measuring.
    ...
    def predict(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        # L_graph — lead-lag carrier (bit-identical to submission_ticket2)
        pt2 = self._g.predict(features)
        L_graph = pt2[tk].to_numpy(dtype=np.float64)   # identical ticker order
        # L_ml — learned leg (bit-identical to submission_apex6)
