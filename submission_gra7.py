"""Competition 5 submission — THREE legs at equal risk, no fitted weights.

    sig = L_graph / SCALE_GRAPH + L_resid / SCALE_RESID + L_apex / SCALE_APEX,
    then row de-mean.

Three structurally independent families, each divided by its own realised PnL
volatility so all three contribute equal risk, then summed with unit
coefficients.  There is no blend weight to choose and nothing is measured at
predict time.

  L_resid = -csrank of the per-timestamp cross-sectional OLS residual of
            Feature.1 on [1, F2..F6].  Purely row-local: it reads one
            timestamp's cross-section and nothing else, so it carries no state
            and cannot look forward.

  L_graph = two-graph lead-lag signal: a grid-averaged VAR(1) lead-lag graph in
            price space over seven trailing windows, plus a rank-space
            own-name-zeroed ridge VAR over two trailing tails, each scaled to
            unit cross-sectional dispersion per row and summed 1:1.  The `_legA`
            body is a vectorised form of the row-at-a-time recursion, verified
            equal to it at max|delta| = 0.0.

  L_apex  = 0.60 * an expanding cross-sectional core (with a corrected inner
            beta projection onto the row-demeaned beta column) + 0.20 * a
            four-model LightGBM ensemble + 0.20 * a per-ticker autocorrelation
            leg, then a rolling-beta projection of the combined forecast, then a
            risk-model position map (rolling 250-bar covariance with analytic
            Ledoit-Wolf shrinkage; w = C^-1 f, renormalised to the input row L2
            norm and blended 50/50 with the untransformed forecast).

WHY THESE THREE, AT EQUAL RISK.  They are the three most structurally
different families available -- a lead-lag graph, a row-local cross-sectional
residual, and a learned/risk-mapped leg -- and w = 1 on each is the canonical
equal-risk point: zero free parameters.  No weight was swept and no argmax was
taken.

SCALES are FROZEN CLASS CONSTANTS (per-leg realised PnL volatility over the
first 75 periods, ddof=0, measured once offline).  train() prepares warm-up
state only and measures nothing over the scored block.

CAUSALITY: train() reads training rows only.  L_graph recurses causally forward
one row at a time from a training-row warm-up buffer.  L_apex uses only
expanding / rolling statistics warm-started from training-row sufficient
statistics, and every normalisation inside it is row-wise, so no statistic
computed over the scored block can feed an earlier row.  csrank and de-mean are
per-timestamp cross-sectional operations.  No backward fill, no negative shift,
no whole-block statistic.  L_resid is computed independently per row.
...
class Gra7Predictor(Predictor):
    """Lead-lag graph + cross-sectional residual + risk-mapped learned leg,
    frozen equal risk."""
    ...
    SCALE_APEX = 0.019215743864648176   # frozen-scale: apex leg PnL vol
    ...
    #  Copied verbatim from submission_triad.py.
    # ====================================================================
    @staticmethod
    def _tickers(features):
        cols = [c for c in features.columns if c[0] == "Feature.1"]
