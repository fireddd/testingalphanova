...
# ///
"""Competition 5 — basic gradient-boosted cross-sectional ranker (CatBoost, 1-bar target).
...
class GbmCat1BarPredictor(Predictor):
    """One CatBoost model on rank features -> csrank -> row de-mean."""
    ...
    def train(self, features: pd.DataFrame, target: pd.DataFrame) -> None:
        ...
        # `target` is the 5-BAR forward cumulative return (per-lag corr ~0.43 for lags 1..5,
        # ~0.00 from lag 6) while scoring is 1-BAR. Feature.1 IS the contemporaneous return,
        # so the next row's Feature.1 is the 1-bar forward return. Using it as the label
        # matches the training objective to the scoring horizon. Train-time only: the shift
        # happens entirely inside the rows handed to train(), never in predict().
        f1 = self._mat(features, 1, tk)
        nxt = np.full_like(f1, np.nan); nxt[:-1] = f1[1:]
        y = self._csr(np.nan_to_num(nxt))
        T = X.shape[0]
        ...
        good[-X.shape[1]:] = False   # drop the final row: no next bar exists
