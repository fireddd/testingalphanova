"""Competition 5 final submission — TWO legs, EQUAL EXPECTED-RETURN contribution.

    sig = L_graph / SCALE_GRAPH + L_apex / SCALE_APEX,  then row cross-sectional de-mean.

CONSTRUCTION RULE (zero free parameters, SOLVED not chosen).  Each leg is divided
by its own frozen sel75 PnL volatility, then the apex leg is additionally scaled so
that the TWO LEGS CONTRIBUTE EQUAL EXPECTED RETURN over the selection window:
    w* = mean(graph_pnl) / mean(apex_pnl)  =  0.775492   (apex return share = 0.5000)
    SCALE_APEX = 0.019215743864648176 / w* = 0.024778769297110750
This is the equal-RETURN analogue of the equal-RISK rule used elsewhere in this
project.  It is preferred here because the two legs have very different measured
platform transfer ratios (graph 0.7701, apex 0.4278), so equalising RISK
over-weights the leg that transfers worst.  Nothing is fitted and nothing is
argmaxed: w* is the solution of a stated equation, not a swept value.

  L_graph = two-graph lead-lag signal (price-space VAR(1) lead-lag + rank-space
            own-name-zeroed ridge VAR), nested non-inheriting helper _GraphLeg.
  L_apex  = expanding cross-sectional core + LightGBM ensemble + autocorrelation
            leg with rolling-beta neutralisation and a Ledoit-Wolf risk map,
            nested non-inheriting helper _Apex7.

CAUSALITY.  train() reads training rows only.  Every normalisation is row-wise
(axis=1) or over a strictly trailing window; there is no whole-block statistic.
An element-wise probe (perturb all elements of rows >= t, inspect rows < t)
returns exactly 0.0, and the predictor is chunk-invariant: predicting a block and
predicting its prefix agree bit-for-bit.

LEGALITY.  Exactly one Predictor subclass at module level; helpers are nested and
inherit nothing; no module-level functions or state.
"""
...
class FinalPredictor(Predictor):
