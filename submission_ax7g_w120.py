"""Competition 5 submission — TWO legs at equal risk, no fitted weights.

    sig = L_graph / SCALE_GRAPH + L_apex / SCALE_APEX,  then row de-mean.

Two structurally independent families, each divided by its own realised PnL
volatility so both contribute equal risk, then summed with unit coefficients.
There is no blend weight to choose and nothing is measured at predict time.
...
WHY THESE TWO, AT EQUAL RISK.  They are the two most structurally different
families available, and w = 1 is the canonical equal-risk point: zero free
parameters.  A risk-share sweep shows a broad plateau around it; the sweep
maximum lies elsewhere and is deliberately NOT taken, because selecting a swept
argmax is exactly the behaviour that this project has measured as costly.
...
class Ax7GraphW120Predictor(Predictor):
    """Lead-lag graph + risk-mapped learned leg, frozen equal risk."""
    ...
    class _Apex7:
        ...
        COV_WIN = 120          # same window apex6 already uses for its rolling beta
