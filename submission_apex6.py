"""Competition 5 submission: APEX6 — three-pipeline combination.

  0.60  Apex4 core   (5 warm-started legs + betaneut + 0.25 reversal-neut,
                      local runner 0.1047)
  0.20  GBM leg      (4-model recency-weighted LightGBM ensemble,
                      next-rank-of-F1 target; local 0.0625, corr .42)
  0.20  TF leg       (per-ticker autocorrelation heterogeneity: each name
                      traded in the direction its own warm-started AC1
                      implies, orthogonalized off plain reversal;
                      local 0.0402, corr vs core .095)
then one extra rolling-beta projection of the combined signal, row-demean.
Scale constants are fixed offline (median per-period cross-sectional std of
each leg) so nothing at predict time uses slice-level statistics.

Local evidence (2026-07-21): combined full 0.1110, sel75 0.1115, h2 0.0690,
sealed-window 0.0959; blocked 7-fold CV 6/7 wins vs Apex4; bootstrap
P(combined > Apex4) = 1.000 over 2000 period resamples.
"""
...
class Apex6Predictor(Predictor):
