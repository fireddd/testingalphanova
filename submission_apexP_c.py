"""Competition 5 submission: APEX-P — APEX6 plus risk-model position mapping.

Identical forecast machinery to APEX6:

  0.60  Apex4 core   (5 warm-started legs + betaneut + 0.25 reversal-neut)
  0.20  GBM leg      (4-model recency-weighted LightGBM ensemble)
  0.20  TF leg       (per-ticker autocorrelation heterogeneity)
then one extra rolling-beta projection of the combined signal.

NEW (and the only difference): the final forecast vector is mapped to
positions through an inverse covariance instead of being used directly.
Kakushadze eq. (59): the unconstrained Sharpe-maximising holdings are
w = C^-1 * forecast, not the forecast itself.  C is a causal rolling
250-bar covariance of Feature.1 across the 20 names with Ledoit-Wolf
shrinkage toward a scaled identity (the LW intensity is computed
analytically from the data — no fitted hyper-parameter).  The mapped
vector is rescaled to the row L2 norm of the input (so the change is
purely directional — no implicit per-bar vol timing) and blended 50/50
with the untransformed forecast, i.e. shrinkage of the risk model toward
"no risk model".  Then row-demean.

Only two new constants: LAM = 0.50 (blend, flat plateau 0.25-0.70) and
COV_WIN = 250 (the same window apex6 already uses for its rolling beta).

Local evidence (2026-07-21, prediction-level replay of the 77-period
walk-forward): full 0.1158 vs apex6 0.1115; sel75 0.1163 vs 0.1121;
h1 0.1484 vs 0.1454; h2 0.0745 vs 0.0683; rec20 0.0901 vs 0.0852;
sealed 076/077 0.1002 vs 0.0942.  Blocked 7-fold CV 7/7 folds beat
apex6; period bootstrap (2000 resamples of the 75 selection periods)
P(apexP > apex6) = 0.995.  pnl-corr vs apex6 0.964, vs sparse 0.373
(apex6 0.400), vs unique 0.287 (apex6 0.296).

Harness (runner.py --full --gauge-fix): Sharpe 0.1153 (apex6 0.1115),
IC 0.0414 (apex6 0.0446), concentration 0.0587, city novelty 35.5 deg
(apex6 35.2, nearest nonius_0271).

Plateau evidence (full Sharpe, all 15 cells > apex6's 0.1115):
  cov window   125     250     500    1000    2000
  lam 0.3    0.1152  0.1149  0.1140  0.1140  0.1133
  lam 0.5    0.1164  0.1158  0.1140  0.1141  0.1127
  lam 0.7    0.1159  0.1148  0.1121  0.1122  0.1100
"""
...
class ApexPPredictorC(Predictor):
