...
weighting scheme.  Nothing here is fitted, tuned, or argmaxed.
  ...
  L_apex6 = submission_apex6.Apex6Predictor's signal (0.60 apex4 core +
            ...
            combined rolling-beta neutralisation), inlined verbatim as the
            non-inheriting nested helper `_Apex6`.  Copied BYTE-FOR-BYTE from the
            shipped, platform-ACCEPTED apex6 (platform 0.0477), including the
            known cosmetic un-axised `pm.std()` in its GBM leg — "fixing" that
            would change the signal and invalidate the shipped anchor.
...
class ApexFusionPredictor(Predictor):
    ...
    # ====================================================================
    #  apex6 sleeve — BYTE-FOR-BYTE copy of submission_apex6.Apex6Predictor
    #  (re-indented only), stripped of the `Predictor` base so that this
    #  module still exposes exactly ONE Predictor subclass.  Do NOT "fix"
    #  the un-axised `pm.std()` in _GBM.predict: apex6 SHIPPED with it and
    #  was ACCEPTED at platform 0.0477.
    # ====================================================================
    class _Apex6:
