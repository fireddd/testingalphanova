"""Competition 5 — FOUR structurally disjoint views, frozen equal-risk sum.

    sig = graph/SCALE_GRAPH + apex/SCALE_APEX + innov/SCALE_INNOV + distr/SCALE_DISTR

Each leg is a different *kind* of statement about the cross-section, and the four are
close to mutually orthogonal in realised PnL (largest pairwise |corr| 0.144). That is
the entire design: a sum of near-orthogonal views keeps the strong legs' edge while
diluting any single leg's exposure to a crowded axis.

  graph  lead-lag propagation  — who moves before whom
  apex   learned/ML composite  — the flagship stack
  innov  fast innovation in Feature.2 / Feature.5 / Feature.6, with each name's own
         trailing 2-bar z-level subtracted, so only the *surprise* survives. Uses no
         Feature.1 at all.
  distr  distributional / second-moment shape of the recent return path.

All four scales are FROZEN CLASS CONSTANTS (per-leg realised PnL volatility over the
first 75 periods, ddof=0, measured once offline). Unit coefficients on graph and apex;
the two exploratory legs enter at half risk. No fitted weights, no argmax.

CAUSALITY. Every leg is row-local or strictly trailing; no statistic is computed over
the prediction block. An element-wise probe returns exactly 0.0 before the perturbation
point, and the predictor is chunk-invariant.

LEGALITY. One Predictor subclass at module level; every helper nested and inheriting
nothing; no module-level functions or state.
"""
...
class QuadPredictor(Predictor):
    """Four disjoint views, frozen equal-risk sum, zero fitted weights."""

    SCALE_GRAPH = 0.04933074223312276
    SCALE_APEX  = 0.019215743864648176
    SCALE_INNOV = 0.22472727603967144
    SCALE_DISTR = 8.56530468720973381
    ...
    def __init__(self):
        ...
        self._apex  = self._Apex()
        self._innov = self._Innov()
        self._distr = self._Distr()

    def train(self, features: pd.DataFrame, target: pd.DataFrame) -> None:
        for leg in (self._graph, self._apex, self._innov, self._distr):
    ...
    def predict(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        g = self._graph.predict(features)[tk].to_numpy(dtype=np.float64)
        a = self._apex.predict(features)[tk].to_numpy(dtype=np.float64)
        i = self._innov.predict(features)[tk].to_numpy(dtype=np.float64)
        d = self._distr.predict(features)[tk].to_numpy(dtype=np.float64)
        sig = (g / self.SCALE_GRAPH + a / self.SCALE_APEX
               + i / self.SCALE_INNOV + d / self.SCALE_DISTR)
