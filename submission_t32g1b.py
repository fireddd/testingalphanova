"""t32: graph leg standalone, leg B refitted to the 1-BAR FORWARD label.

ONE CHANGE vs the shipped graph leg (submission_ticket2.py / _GraphLeg inside
submission_final.py): leg B's ridge-VAR regression label.

    BEFORE   y[t] = rowstd( target[t] )   -- target is the de-meaned 5-BAR
                                             forward cumulative return
    AFTER    y[t] = rowstd( f1[t+1] )     -- the realised 1-BAR forward return

Scoring is 1-bar (`predictions.shift(1) * returns`), so the shipped leg B was
optimising a horizon it is not scored on.  De-meaned Feature.1 is the de-meaned
contemporaneous return, so the NEXT training row's Feature.1 is the 1-bar forward
return.  The shift happens entirely inside the rows handed to train(); predict()
never shifts anything, so the change is causally free.

Leg A is untouched.  WGRID, LAM_A, MINP, TAILS, LAM_B, KLAG, HM are unchanged.
"""
...
class Graph1BarBPredictor(Predictor):
    """Leg A (unsupervised VAR(1) grid) + leg B (ridge VAR on the 1-bar label)."""
    ...
    def train(self, features, target):
        ...
        # LABEL (the one change): the 1-BAR FORWARD return, i.e. the NEXT
        # training row's Feature.1.  Built here, inside train(), from training
        # rows only; the last row has no successor and is marked NaN so it can
        # never enter the design.  predict() does no shifting whatsoever.
        tgt = np.full_like(f1, np.nan)
        tgt[:-1] = f1[1:]
        ntr = len(f1)
        kj = self.KLAG * j
    ...
    def predict(self, features):
        ...
        sig = self._unit_row(b)  # LEG B ONLY (diagnostic build)
